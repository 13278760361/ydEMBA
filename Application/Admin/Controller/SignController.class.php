...
class SignController extends AdminController {
  function _initialize(){
    parent::_initialize();
        ...
        $this->student_no  = I('get.student_no');
	...
	public function index(){
     ...
     $subject_id =I('subject_id');
      ...
      $field="s.id,cch.class_hour_id,s.username,s.student_no,c.class_name,y.year_name,su.subject_name,ch.start_time,ch.end_time";
      ...
      $_data = I('post.');
        ...
        if ( M('Sign_history')->where( array('user_id'=>$_data['user_id'],'class_hour_id'=>$_data['class_hour_id'],'sign_type'=>1) )->find() ) {
         ...
         if ($res !== false && $res2 !== false && $res3 !==false ) {
           ...
           $username = M('Students')->where( array('student_no'=>$this->student_no) )->getField('username');
           ...
           $sinfo = M("students")->where(array('student_no'=>$this->student_no))->find();
           $s_sign_time = M('Sign_history')->where( array('user_id'=>$sinfo['id'],'class_hour_id'=>$this->class_hour_id,'sign_type'=>1) )->getField('sign_time');

           $s_sign_time = !empty($s_sign_time)?$s_sign_time:'';
       
           $e_sign_time =  M('Sign_history')->where( array('user_id'=>$sinfo['id'],'class_hour_id'=>$this->class_hour_id,'sign_type'=>2) )->max('sign_time');
