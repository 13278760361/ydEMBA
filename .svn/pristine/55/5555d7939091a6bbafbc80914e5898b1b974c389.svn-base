    ...
    public function index(){
         ...
         if (  $this->db->where(array('openid'=>$openid))->find()  ) { //如果有数据 执行打卡操作
              ...
              $where ="st.openid ='{$openid}' and cr.class_room_name ='{$classR}' and  
               ...
               ROUND( UNIX_TIMESTAMP(DATE_ADD(FROM_UNIXTIME(end_time, '%Y-%m-%d %H:%i:%s'),INTERVAL 60 MINUTE)))  >= {$now_time} and st.id = cc.user_id and cc.class_hour_id = c.id and c.class_room_id = cr.id";
                    ...
                    if ( $first_sign =   $this->db_sign->where(array('user_id'=>$dk['user_id'],'class_hour_id'=>$dk['class_hour_id'],'sign_type'=>1))->find() ) { //查看是否有过打卡记录
                        ...
                        if ($res) {
                              $this->db->commit();
                                ...
                                if (  $now_time >  mktime(8,0,0,date("m",time()),date("d",time()),date("Y",time()))  && $now_time < mktime(11,0,0,date("m",time()),date("d",time()),date("Y",time()))) { //再次时间内，跳转投票打卡页面
                                    $url = U('Public/signVSuccess',array('subject_id'=>$dk['subject_id'],'teacher_id'=>$dk['teacher_id'],'classR'=>$classR ));
                                }else{
                        ...
                        //添加打卡记录 
                         ...
                         if ($res && $res_2 && $res_3) {
                            $this->db->commit();
                              ...
                              if (  $now_time >  mktime(8,0,0,date("m",time()),date("d",time()),date("Y",time()))  && $now_time < mktime(11,0,0,date("m",time()),date("d",time()),date("Y",time()))) { //再次时间内，跳转投票打卡页面
                                    $url = U('Public/signVSuccess',array('subject_id'=>$find_dk['subject_id'],'teacher_id'=>$find_dk['teacher_id'],'classR'=>$classR ));
                                }else{
